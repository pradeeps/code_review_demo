...
def write_file_stats(device, LOGS_FOLDER, VIF_INDEX, suffix=None):
    ...
    try:
        rx_successes = (
            os.popen(adb_cmd, "".join([" 2206." + str(i) for i in range(1, 37)]))
            .read()
            .splitlines()
        )
        rx_successes.pop(0)
        tx_successes = (
            os.popen(adb_cmd, "".join([" 2207." + str(i) for i in range(1, 37)]))
            .read()
            .splitlines()
        )
        tx_successes.pop(0)
        tx_retries = (
            os.popen(adb_cmd, "".join([" 2207." + str(i) for i in range(71, 107)]))
            .read()
            .splitlines()
        )
        tx_retries.pop(0)
        ...
        for i in range(len(rx_successes)):
            ...
            if tx_retries[i] + tx_successes[i] > 0:
                tx_retries_per.append(
                    (float(tx_retries[i]) / (tx_retries[i] + tx_successes[i])) * 100
                )
            else:
        ...
        summary = (
            os.popen(
                adb_cmd
                + " 2207.37 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz 2207.zz "
            )
            .read()
            .splitlines()
        )
        ...
        if len(summary) == 0:
            ...
            if suffix:
                results_file = open(
                    LOGS_FOLDER + "/rates_stats_" + device + "_" + suffix + ".txt", "w"
                )
            else:
                results_file = open(
                    LOGS_FOLDER + "/rates_stats_" + device + ".txt", "w"
                )
